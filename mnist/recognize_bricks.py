...
def make_image_tensor(input_batches):
  labels, input_arrays = input_batches
  new_arrays = []
  for array in input_arrays:
    new_arrays.append(array.reshape(1, INPUT_HEIGHT, INPUT_WIDTH))
...
def main():
  ...
  in_sample = T.dtensor4('pixels')
  target = T.imatrix('labels')
  ...
  main_loop = blocks.main_loop.MainLoop(
      ...
      extensions=[
          ...
          blocks.extensions.saveload.Checkpoint(
              'mnist_nn.dat', after_epoch=True),
          blocks_extras.extensions.plot.Plot(
              'Plotting MNIST learning',
              channels=[
